#!/usr/bin/env python3
"""End-to-end demo, guard in the middle between a client and DeepSeek.

    .venv/bin/python guard/tools/demo.py                    # mock upstream, no key needed
...
attacks must come back 403 and never get forwarded. "penetration testing" is a
regression sample: the stock questions flagged it as a jailbreak, it has to
keep coming back allowed.
"""
import argparse
import json
import os
        ...
        payload = {
            ...
            "choices": [{
                ...
                "message": {"role": "assistant",
                            "content": "[mock upstream] %d message(s); last: %s" % (len(messages), last[:140])},
                "finish_reason": "stop",
            }],
...
def start_mock_upstream(port):
    server = ThreadingHTTPServer(("127.0.0.1", port), _MockHandler)
    threading.Thread(target=server.serve_forever, daemon=True).start()
    return server


def start_guard(port, upstream_base):
    # app.py reads env at import, so set the upstream before pulling it in
...
def wait_ready(url, timeout=30.0):
    ...
    while time.time() < deadline:
        try:
            ...
            last_error = exc
        time.sleep(0.4)
    raise SystemExit("guard server never came up: %s" % last_error)


def check_one(client, base, label, prompt, mode):
...
def main():
    ...
    if args.url:
        ...
        use_mock = args.mock or not os.environ.get("DEEPSEEK_API_KEY")
    ...
    if not args.url:
        print("[demo] first request builds the Laya checkpoint, ~20s on cpu", flush=True)

    samples = [("single prompt", args.prompt)] if args.prompt else SAMPLES
    mode = args.mode
    failures = 0
    ...
    with httpx.Client() as client:
        if args.check:
            for label, prompt in samples:
                verdict = client.post(base + "/v1/guard/check", json={"input": prompt},
                                      headers={"X-Guard-Mode": mode}, timeout=180.0).json()
    ...
    print("\n[demo] %d checked | %d blocked | %d forwarded%s"
          % (len(results), blocked, len(results) - blocked,
             (" | %d flagged" % flagged) if flagged else ""), flush=True)
    if failures:
